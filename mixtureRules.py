...
Reference:
    [1] Lei Lei, Michael P. Burke. Bath gas mixture effects on multichannel reactions:  
        In-sights and representations for systems beyond single-channel reactions. 
        J. Phy. Chem.A, 123(3):631–649, 2019.
    [2] Lei Lei, Michael P. Burke. Evaluating the performance of bath gas mixture rules 
        forgeneral implementation in chemically reacting flow codes: Tests for multi-well, 
        multi-channel reactions. 12th U.S. National Combustion Meeting, Texas, USA, 2021.
...
def NMR_R(T, P, X, k_0, f, k_P):
    """
        ...
        Input:
            ...
            P: float, local pressure, atm;
            X: list of shape (M,), local mixture composition;
            k_0: list of shape (M,), rate coefficients in the low-pressure limit for each component;
    ...
    k_0_mix = np.sum(np.array(X) * np.array(k_0) * np.array(f))
    P_i = P * k_0_mix / (np.array(k_0))
    x_tilde = np.array(k_0) * np.array(X) * np.array(f) / k_0_mix
    ...
    for i,p in enumerate(P_i):
        ...
        # if 2d-arraies are used, interpolate using PLOG formula
            ...
            # interpolate using PLOG form
            k_temp = 10 ** (np.log10(k_P[i][1,P_loc]) + (np.log10(k_P[i][1,P_loc+1]) - np.log10(k_P[i][1,P_loc])) / 
                           (np.log10(k_P[i][0,P_loc+1]) - np.log10(k_P[i][0,P_loc])) * (np.log10(P) + np.log10(k_0_mix) - np.log10(k_P[i][0,P_loc]) - np.log10(k_0[i])))
            temp.append(k_temp)
...
if __name__ == '__main__':
    ...
    k_0 = [4.005782458300001e-24, 8.009631798840002e-23] # rate coefficients in the low-pressure limit for each collider
    k_NMR = NMR_R(T,P,X,k_0,f_list,k_P) # calculate NMR,R rate constant
